...
source("sim.catRT.R")
source("NimbleModel catRT Poisson Multisession.R")
source("NimbleFunctions catRT Poisson Multisession.R")
...
Nimdata <- list(y.ID=nimbuild$y.ID,y.true=array(NA,dim=c(N.session,max(M),max(J))),
              ...
              X=nimbuild$X,capcounts=matrix(NA,N.session,max(M)),G.true=G.true.data,capcounts.ID=capcounts.ID)

# set parameters to monitor
parameters <- c('lam0.fixed','sigma.fixed','theta.thin','N','n','D','lambda','gammaMat')
nt <- 1 #thinning rate
